"""
 * @title Papaya DeFi Lending Platform
 * @author Allan Robinson
 * @notice Main application file for Papaya - AAVE-style lending protocol on XRPL
 * @dev Created: January 9, 2026
 * Core Features:
 * - Collateralized lending (50% LTV, 200% collateralization)
 * - Multi-asset support (XRP, BTC, ETH, SOL, USDC, USDT, KSH)
 * - Real-time price feeds (CoinGecko, ExchangeRate API)
...
def log_transaction(user_id, tx_type, asset=None, amount=None, from_asset=None, to_asset=None, tx_hash=None, status="completed"):
    """
    Log transaction to database for history tracking

    tx_type: 'deposit', 'withdraw', 'borrow', 'repay', 'swap', 'liquidation'
    tx_hash: XRPL transaction hash (if applicable)
    """
    import hashlib
    import time

    # Generate mock transaction hash if not provided (for demo purposes)
    if not tx_hash:
        hash_input = f"{user_id}{tx_type}{asset or ''}{amount or 0}{time.time()}"
        tx_hash = hashlib.sha256(hash_input.encode()).hexdigest()[:64]

    try:
        conn, cursor = get_db_cursor()
        cursor.execute("""
            INSERT INTO transactions (user_id, tx_type, asset, amount, from_asset, to_asset, tx_hash, status)
            VALUES (?, ?, ?, ?, ?, ?, ?, ?)
        """, (user_id, tx_type, asset, amount, from_asset, to_asset, tx_hash, status))
        conn.commit()
        conn.close()
        return tx_hash
...
def register():
    if request.method == 'POST':
        ...
        if user:
            ...
            return redirect(url_for('mm.register'))

        # SIMPLIFIED REGISTRATION: Auto-create XRPL testnet wallet
        try:
            from xrpl.wallet import Wallet
            import requests
            ...
            # Fund the wallet automatically from testnet faucet
            try:
                faucet_response = requests.post(
                    'https://faucet.altnet.rippletest.net/accounts',
                    json={'destination': wallet_address},
                    timeout=10
                )
                funded = faucet_response.status_code == 200
            except:
                funded = False

            # Create user account
            cursor.execute("INSERT INTO users (_id, username, password) VALUES (?, ?, ?)",
                          (wallet_address, username, password))

            # Initialize asset balances
            for asset in ['KSH', 'USDC', 'USDT', 'XRP', 'ETH', 'BTC', 'SOL']:
                cursor.execute("INSERT INTO user_assets (user_id, asset, balance) VALUES (?, ?, ?)",
                              (wallet_address, asset, 0))

            conn.commit()
            conn.close()

            # Show success page with wallet credentials
...
def dashboard(user_id):
    ...
    # Get REAL XRP balance from XRPL testnet (not database)
    try:
        from xrpl.clients import JsonRpcClient
        from xrpl.models import AccountInfo

        client = JsonRpcClient("https://s.altnet.rippletest.net:51234")
        account_info = AccountInfo(account=user_id)
        response = client.request(account_info)

        if response.is_successful():
            balance_drops = response.result['account_data']['Balance']
            real_xrp_balance = int(balance_drops) / 1_000_000  # Convert drops to XRP
            total_assets['XRP'] = real_xrp_balance  # Override with real balance
    except Exception as e:
        print(f"Warning: Could not fetch XRP balance from testnet: {e}")
        # Keep database value if query fails
...
@mm_blueprint.route('/get_demo_tokens', methods=['POST'])
@login_required
def get_demo_tokens():
    """Give users demo tokens for testing (hackathon feature)"""
    try:
        conn, cursor = get_db_cursor()

        # Demo amounts for each asset
        demo_amounts = {
            'KSH': 100000,  # 100,000 KSH
            'USDC': 10000,   # 10,000 USDC
            'USDT': 10000,   # 10,000 USDT
            'XRP': 0,        # XRP comes from testnet faucet
            'ETH': 5,        # 5 ETH
            'BTC': 0.5,      # 0.5 BTC
            'SOL': 100       # 100 SOL
        }
        ...
        for asset, amount in demo_amounts.items():
            cursor.execute(
                "UPDATE user_assets SET balance = balance + ? WHERE user_id = ? AND asset = ?",
                (amount, current_user.id, asset)
            )

        conn.commit()
        conn.close()

        flash('✅ Demo tokens added to your account! Check your dashboard.', 'success')
        return redirect(mm_url_for('dashboard', user_id=current_user.id))

    except Exception as e:
        flash(f'Error adding demo tokens: {str(e)}', 'danger')
        return redirect(mm_url_for('index'))
...
def swap():
    ...
    try:
        ...
        client = JsonRpcClient("https://s.altnet.rippletest.net:51234")
        account_info = AccountInfo(account=current_user.id)
        response = client.request(account_info)

        if response.is_successful():
            balance_drops = response.result['account_data']['Balance']
            real_xrp_balance = int(balance_drops) / 1_000_000  # Convert drops to XRP
            user_balances['XRP'] = real_xrp_balance  # Override with real balance
    except Exception as e:
        print(f"Warning: Could not fetch XRP balance from testnet: {e}")
        # Keep database value if query fails
    ...
    if form.validate_on_submit():
        ...
        if from_asset == to_asset:
            flash('Cannot swap the same asset!', 'danger')
            return redirect(mm_url_for('swap'))

        # Check if user has enough balance
        from_balance = user_balances.get(from_asset, 0)
        if from_balance < amount:
            flash(f'Insufficient {from_asset} balance! You have {from_balance:.6f} {from_asset}', 'danger')
            return redirect(mm_url_for('swap'))

        try:
            # Calculate swap using real-time rates
            # Convert from_asset to USD first, then USD to to_asset
            from_asset_usd_price = exchange_rates.get(from_asset, 1.0)
            to_asset_usd_price = exchange_rates.get(to_asset, 1.0)

            # Calculate USD value of input amount
            usd_value = amount * from_asset_usd_price

            # Calculate output amount
            received_amount = usd_value / to_asset_usd_price

            # Apply 0.3% swap fee
            fee = received_amount * 0.003
            received_amount = received_amount - fee
            ...
            cursor.execute(
                "UPDATE user_assets SET balance = balance - ? WHERE user_id = ? AND asset = ?",
                (amount, current_user.id, from_asset)
            )

            # Add to_asset
            cursor.execute(
                "UPDATE user_assets SET balance = balance + ? WHERE user_id = ? AND asset = ?",
                (received_amount, current_user.id, to_asset)
            )
            ...
            flash(f'✅ Swapped {amount:.6f} {from_asset} → {received_amount:.6f} {to_asset}<br><small>TX: {tx_hash[:16]}...</small>', 'success')
            return redirect(mm_url_for('dashboard', user_id=current_user.id))

        except Exception as e:
            flash(f'Swap failed: {str(e)}', 'danger')
            return redirect(mm_url_for('swap'))

    return render_template('swap.html', title='Swap', form=form, user_balances=user_balances, exchange_rates=exchange_rates)

@mm_blueprint.route('/history', methods=['GET'])
@login_required
def transaction_history():
    """Display user's transaction history with clickable links to XRPL explorer"""
    conn, cursor = get_db_cursor()

    # Fetch all transactions for current user, most recent first
    cursor.execute("""
        SELECT id, tx_type, asset, amount, from_asset, to_asset, tx_hash, status, timestamp
        FROM transactions
        WHERE user_id = ?
    ...
    for tx in transactions:
        formatted_transactions.append({
            ...
            'status': tx[7],
            'timestamp': tx[8],
            'explorer_link': f"https://testnet.xrpl.org/transactions/{tx[6]}" if tx[6] else None
        })
